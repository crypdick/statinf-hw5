...
# excluding intercept coeff, freq that params for predictors had p-val > 0.05 in all our sims. on interval (0,1)
  ...
  payload <- list(sim_results=sim_results, linear_model=my_lm, X=X, Y=Y, eps=eps, Y_hat_ci=Y_hat_ci, exp_val_Y=exp_val_Y, n=sample_size)
  return(payload)
}
```
...
plot_diagnostics <- function(payload) {
  ...
  multiple_predictors <- is.matrix(X)
  ...
  if(multiple_predictors){
    ...
    writeLines("Is there Evidence of Multicollinearity?\nIs the relationship between each predictor and Y linear?")
    pairs(multivariate_dataframe)
    
  } else {
      writeLines("Is the Relationship Between X and Y Linear? ")
      plot(Y ~ X)
      abline(linear_model)
  }
  ...
  writeLines("Do the Residuals Have Constant Variance?\n\n Does the plot appear to be a general cloud of points or is there a megaphone shape?")
  plot(linear_model, which = 1) 
  
  writeLines("Are the Residuals Normally Distributed?")
  plot(linear_model, which = 2)
  
  writeLines("Are the Residuals Independent?")  
  plot(1:n, residuals(linear_model))
...
advanced_diagnostics <- function(payload) {
  ...
  print(shapiro.test(residuals(linear_model)))
...
q1_random_results_1 <- run_simulations(num_runs = 1, 
                                  n, 
                                  num_new_obsv, 
                                  violation =  q1_random_violation_1,
                                  violation_func = q1_random_func[[1]],
                                  num_predictors = 1, 
                                  sigma_sq)


q1_random_violation_2 <- sample(c("None","Nonlinear relationship"), 1)
q1_random_results_2 <- run_simulations(num_runs = 1, 
                                  n, 
                                  num_new_obsv, 
                                  violation =  q1_random_violation_2,
                                  violation_func = q1_random_func[[2]],
                                  num_predictors = 1, 
                                  sigma_sq)


q1_random_violation_3 <- sample(c("None","Nonlinear relationship"), 1)
q1_random_results_3 <- run_simulations(num_runs = 1, 
                                  n, 
                                  num_new_obsv, 
                                  violation =  q1_random_violation_3,
                                  violation_func = q1_random_func[[3]],
                                  num_predictors = 1, 
                                  sigma_sq)
q1_random_results_1[[1]]
q1_random_results_2[[1]]
q1_random_results_2[[1]]
```
...
Based on the diagnostics, we predict that the problem areas that we mentioned in part d will/will not be affected. 
```{r}
q1_random_violation_1
q1_random_func[[1]]
q1_random_violation_2
q1_random_func[[2]]
q1_random_violation_3
q1_random_func[[3]]
```
...
random_results2 <- run_simulations(num_runs = 1, 
                                  n, 
                                  num_new_obsv, 
...
random_results3 <- run_simulations(num_runs = 1, 
                                  n_large, 
                                  num_new_obsv, 
...
## PART E
```{r correlated_error_random_sim}
#randomly select a value of rho equal to .7 or 0 from a large sample size
rho <- sample(c(0.2,0.8),1)
n <- sample(20:5000, 1)
random.violation4 <- sample(c("None","Correlated Errors"),1)
random_results4 <-run_simulations(num_runs = 1,
                                    n,
                                    num_new_obsv,
                                    violation =  random.violation4,
                                    violation_func = rho,
                                    num_predictors = 1,
                                    sigma_sq)
random_results4[[1]]
...
```{r}
plot_diagnostics(random_results4)
```
...
#part e, name the normal func & nonnormal func & randomly call it
n <- sample(c(n_small,n_med,n_large), 1)
random.violation5 <- sample(c("None","Multicolinearity"), 1)

random_results5 <- run_simulations(num_runs = 1,
                                  n,
                                  num_new_obsv,
