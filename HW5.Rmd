...
```{r}
yhat_med <- data.frame(yhat_med)

ggplot(yhat_med, aes(x =locations_med$x_med, y = fit)) +
  geom_point(size = 4) +
  geom_errorbar(aes(ymax = yhat_med$upr, ymin = yhat_med$lwr))
```
TODO
...
```{r}
yhat_large <- data.frame(yhat_large)

ggplot(yhat_large, aes(x =locations_large$x_large, y = fit)) +
  geom_point(size = 4) +
  geom_errorbar(aes(ymax = yhat_large$upr, ymin = yhat_large$lwr))
```
