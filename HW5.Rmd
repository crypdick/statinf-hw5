...
run_simulations <- function(num_runs, sample_size, num_new_obsv, violation, violation_func, parameters, sigma_sq) {
  ...
  sim_results <- data.frame(bad_predictions = 0,
                            bad_b0 = 0, 
                            bad_b1 = 0, 
                            mse = 0, 
                            insignificant_b0 = 0, 
  ...
  for (i in 1:num_runs) {
    ...
    y <- b0 + b1 * violated_x + eps
    exp_val_y <- b0 + b1 * violated_new_obsv
    ...
    good_predictions <- sum((exp_val_y >= y_hat_ci[,2]) & (exp_val_y <= y_hat_ci[,3]))
    if (good_predictions < num_new_obsv/2) sim_results$bad_predictions <- sim_results$bad_predictions + 1
    ...
    if (good_betas[1] == FALSE) sim_results$bad_b0 <- sim_results$bad_b0 + 1
    if (good_betas[2] == FALSE) sim_results$bad_b1 <- sim_results$bad_b1 + 1
    
    # add the mse to the sum of MSE for all runs (to be averaged at the end)
    sim_results$mse <- sim_results$mse + (anova(my_lm)$'Mean Sq'[2])
    
    
    # check which beta_hats are significant based on the p-values
    p_vals <- summary(my_lm)$coeff[,4]
    if (p_vals[1] > .05) sim_results$insignificant_b0 <- sim_results$insignificant_b0 + 1
    if (p_vals[2] > .05) sim_results$insignificant_b1 <- sim_results$insignificant_b1 + 1
  }
  sim_results$mse <- sim_results$mse/num_runs
