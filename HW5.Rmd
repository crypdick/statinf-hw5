...
run_simulations <- function(num_runs, sample_size, num_new_obsv, violation, violation_func, num_predictors, sigma_sq) {
  ...
  sim_results <- data.frame(bad_prediction_rate = 0,
                            bad_intercept_rate = 0, 
                            bad_predictor_rate = 0, 
                            average_mse = 0, 
                            insignificant_intercept_rate = 0, 
                            insignificant_predictor_rate = 0)
  ...
  for (i in 1:num_runs) {
    ...
    new_data <- data.frame(X = new_obsv)
    ...
    if (good_betas[1] == FALSE) sim_results$bad_intercept_rate <- sim_results$bad_intercept_rate + 1
    if (sum(good_betas[-1] == FALSE) >= 1) 
      {sim_results$bad_predictor_rate <- sim_results$bad_predictor_rate + sum(good_betas[-1] == FALSE)}
    ...
    if (p_vals[1] > .05) sim_results$insignificant_intercept_rate <- sim_results$insignificant_intercept_rate + 1
    if (sum(p_vals[-1] > .05) >= 1) 
      {sim_results$insignificant_predictor_rate <- sim_results$insignificant_predictor_rate + sum(p_vals[-1] > .05)}
  }
  sim_results <- sim_results/num_runs
  sim_results$bad_prediction_rate <- sim_results$bad_prediction_rate/(num_new_obsv)
  sim_results$insignificant_predictor_rate <- sim_results$bad_predictor_rate/(num_predictors)
  sim_results$insignificant_predictor_rate <- sim_results$insignificant_predictor_rate/(num_predictors)
...
```{r}
show_diagnostics(poly_small_results)
```

```{r}
show_diagnostics(poly_med_results)
```


```{r}
show_diagnostics(poly_large_results)
```
...
```{r}
show_diagnostics(exp_small_results)
```

```{r}
show_diagnostics(exp_med_results)
```

```{r}
show_diagnostics(exp_large_results)
```
...
```{r}
show_diagnostics(sin_small_results)
```

```{r}
show_diagnostics(sin_med_results)
```

```{r}
show_diagnostics(sin_large_results)
```
...
```{r}
show_diagnostics(log_small_results)
```

```{r}
show_diagnostics(log_med_results)
```

```{r}
show_diagnostics(log_large_results)
```


TODO  check code and write interpretation.

## PART E
```{r}
#part e, name the normal func & nonnormal func & randomly call it
random.func <- sample(c(poly_func, sin, exp, log), 1)
n <- sample(20:5000, 1)
random.violation <- sample(c("None","Nonlinear relationship"), 1)

random_results <- run_simulations(num_runs = 1, 
                                  n, 
                                  num_new_obsv, 
                                  violation =  random.violation,
                                  violation_func = random.func,
                                  num_predictors, 
                                  sigma_sq)
...
```{r}
show_diagnostics(random_results)
```
...
```{r}
show_diagnostics(gamma_small_results)
```

```{r}
show_diagnostics(gamma_med_results)
```

```{r}
show_diagnostics(gamma_large_results)
```
...
```{r}
show_diagnostics(poisson_small_results)
```

```{r}
show_diagnostics(poisson_med_results)
```

```{r}
show_diagnostics(poisson_large_results)
```
...
```{r}
hetero_var_results <- rbind(small_samp = hetero_var_small_results[[1]],
                             med_samp = hetero_var_med_results[[1]], 
                             large_samp = hetero_var_large_results[[1]])
hetero_var_results
```

```{r}
show_diagnostics(hetero_var_small_results)
```

```{r}
show_diagnostics(hetero_var_med_results)
```

```{r}
show_diagnostics(hetero_var_large_results)
```
...
```{r}
multicolinearity_results <- rbind(small_samp = multicolinearity_small_results[[1]],
                             med_samp = multicolinearity_med_results[[1]],
                             large_samp = multicolinearity_large_results[[1]])
multicolinearity_results
```

```{r}
show_diagnostics(multicolinearity_small_results)
```

```{r}
show_diagnostics(multicolinearity_med_results)
```

```{r}
show_diagnostics(multicolinearity_large_results)
```
