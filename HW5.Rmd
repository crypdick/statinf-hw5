...
run_simulations <- function(num_runs, sample_size, num_new_obsv, violation, violation_func, parameters, sigma_sq) {
  ...
  # see if I can get away with stashing lm inside sim
  sim_results.lm <- my_lm
  sim_results.x <- x
  sim_results.y <- y
  sim_results.eps <- eps
  sim_results.y_hat_ci <- y_hat_ci
  sim_results.exp_val_y <- exp_val_y
...
```{r show_diagnostics}
show_diagnostics <- function(lm_model, xs, ys) {
  summary(lm_model)
  anova(lm_model)
  plot(ys ~ xs)
abline(lm_model)
